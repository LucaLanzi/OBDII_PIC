...
# 4 "LCD.c" 2
# 26 "LCD.c"
static uint8_t LCD_display_control = 0x0C;
static uint8_t LCD_entry_mode = 0x06;
...
void LCD_write_command(char command){
    ...
    LATDbits.LATD7 = (command & 0b10000000) >> 7;
    LATDbits.LATD6 = (command & 0b01000000) >> 6;
    LATDbits.LATD5 = (command & 0b00100000) >> 5;
    LATDbits.LATD4 = (command & 0b00010000) >> 4;


    LCD_pulse_clock();


    LATDbits.LATD7 = (command & 0b00001000) >> 3;
    LATDbits.LATD6 = (command & 0b00000100) >> 2;
    LATDbits.LATD5 = (command & 0b00000010) >> 1;
    LATDbits.LATD4 = (command & 0b00000001) ;
...
void LCD_write_data(char data){
    ...
    LATDbits.LATD7 = (data & 0b10000000) >> 7;
    LATDbits.LATD6 = (data & 0b01000000) >> 6;
    LATDbits.LATD5 = (data & 0b00100000) >> 5;
    LATDbits.LATD4 = (data & 0b00010000) >> 4;


    LCD_pulse_clock();


    LATDbits.LATD7 = (data & 0b00001000) >> 3;
    LATDbits.LATD6 = (data & 0b00000100) >> 2;
    LATDbits.LATD5 = (data & 0b00000010) >> 1;
    LATDbits.LATD4 = (data & 0b00000001) ;
...
void LCD_init(){
    ...
    _delay((unsigned long)((6)*(64000000/4000.0)));
...
void LCD_cursor_set(uint8_t row, uint8_t column){
    ...
    if(row == 1) column += (0x80-1);
    if(row == 2) column += (0xC0-1);
    if(row == 3) column += (0x80+19);
    if(row == 4) column += (0xC0+19);
...
void LCD_clear(){
    ...
    _delay((unsigned long)(((2300-60))*(64000000/4000000.0)));
...
void LCD_add_character(char *pattern, uint8_t address){
    ...
    LCD_write_command( 0x40 + (uint8_t)(address<<3) );
...
void LCD_write_variable(int32_t variable, uint8_t left_offset){
    ...
    while(1){
        digits++;
        if(temp_variable < 10) break;
        temp_variable /= 10;
    }


    while(1){
        if(left_offset <= digits) break;
        LCD_write_data(' ');
        left_offset--;
    }
    ...
    while(1){
        ...
        LCD_write_data( (temp_variable%10) + '0' );
        digits --;
        if(digits == 0) break;
    }
}
...
void LCD_write_float(float variable, uint8_t left_offset, uint8_t f_digit){
    ...
    float integer;
    float fractional;
    fractional = modff(variable, &integer);


    LCD_write_variable( (int32_t)integer, left_offset );
    LCD_write_char('.');


    for(uint8_t i=0 ; i < f_digit ; i++){
        fractional *= 10;
        LCD_write_data( (uint8_t)((int32_t)fractional%10 + '0') );
    }
}
